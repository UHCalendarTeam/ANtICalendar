...
";
        ...
        public bool checkProperties(Dictionary<string, IComponentProperty> container1, Dictionary<string, IComponentProperty> container2)
        {
            foreach (var prop in container1)
            {
                if (!container2.ContainsKey(prop.Key))
                    return false;                
                if (container2[prop.Key].StringValue != prop.Value.StringValue)
                    return false; 
            }
            return true;
        }
