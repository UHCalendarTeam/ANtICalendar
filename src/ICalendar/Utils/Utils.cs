        ...
        public static bool ToDateTime(this string stringDate, out DateTime? resDateTime)
        {
            ...
            if (match.Success)
            {
                resDateTime = new DateTime(
                    ...
                    match.Groups[match.Groups.Count - 1].Value.Equals("Z", StringComparison.OrdinalIgnoreCase)
                        ? DateTimeKind.Utc
                        : DateTimeKind.Unspecified);
                return true;
            }
            if (DateTime.TryParse(stringDate, out ret))
            {
                resDateTime = ret;
                return true;
            }
